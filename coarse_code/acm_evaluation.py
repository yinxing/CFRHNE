...
class Acm_evaluation():
    def __init__(self):
        ...
        self.business_label = {}
        self.sample_num = 0
        with open('../data/acm/paper_labels.dat') as infile:
        ...
        self.sample_8 = list()
        self.sample_2 = list()
        with open('../data/acm/acm_ap.test_0.2') as infile:
            for line in infile.readlines():
                u, b, label = [int(item) for item in line.strip().split()]
                self.test_link_label.append([u, b, 1])

        with open('../data/acm/acm_ap.train_0.8') as infile:
            for line in infile.readlines():
                u, b, label = [int(item) for item in line.strip().split()]
                self.train_link_label.append([u, b, 1])

        with open('../data/acm/neg_0.8') as infile:
            for line in infile.readlines():
                u, b, label = [int(item) for item in line.strip().split()]
                self.sample_8.append([u, b, label])

        with open('../data/acm/neg_0.2') as infile:
            for line in infile.readlines():
                u, b, label = [int(item) for item in line.strip().split()]
    ...
    def evaluate_acm_cluster(self, embedding_matrix):
    ...
    def evaluate_acm_classification(self, embedding_matrix): #分类任务
        embedding_list = embedding_matrix.tolist()
    ...
    def acm_link_prediction(self, embedding_matrix):
