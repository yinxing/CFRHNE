...
class DBLP_evaluation():
    def __init__(self):
        self.author2id = {}
        self.author_num = 0
        with open('../data/dblp/dblp_author_map_id.dat') as infile: #id, author 唯一
        ...
        self.author_label = {}
        self.sample_num = 0
        with open('../data/dblp/dblp_author_label.dat') as infile:
            for line in infile.readlines():
        ...
        self.sample_8 = []
        self.sample_2 = []
        with open('../data/dblp/neg_0.8') as infile:
            for line in infile.readlines():
                u, b, label = [int(item) for item in line.strip().split()]
                self.sample_8.append([u, b, label])

        with open('../data/dblp/neg_0.2') as infile:
            for line in infile.readlines():
                u, b, label = [int(item) for item in line.strip().split()]
    ...
    def evaluation_link_prediction(self, embedding_matrix):
        ...
        for a, p, label in self.sample_8:
            train_x.append(embedding_list[a] + embedding_list[p])
            train_y.append(float(label))
        ...
        for a, p, label in self.sample_2:
            test_x.append(embedding_list[a] + embedding_list[p])
            test_y.append(float(label))
