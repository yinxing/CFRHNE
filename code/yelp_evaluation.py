...
class Yelp_evaluation():
    def __init__(self):
        ...
        self.business_label = {}
        self.sample_num = 0
        with open('../data/yelp/yelp_business_category.txt') as infile:
        ...
        self.sample_8 = list()
        self.sample_2 = list()
        with open('../data/yelp/yelp_ub.test_0.2') as infile:
            for line in infile.readlines():
                u, b, label = [int(item) for item in line.strip().split()]
                self.test_link_label.append([u, b, 1])

        with open('../data/yelp/yelp_ub.train_0.8') as infile:
            for line in infile.readlines():
                u, b, label = [int(item) for item in line.strip().split()]
                self.train_link_label.append([u, b, 1])

        with open('../data/yelp/neg_0.8') as infile:
            for line in infile.readlines():
                u, b, label = [int(item) for item in line.strip().split()]
                self.sample_8.append([u, b, label])

        with open('../data/yelp/neg_0.2') as infile:
            for line in infile.readlines():
                u, b, label = [int(item) for item in line.strip().split()]
    ...
    def evaluate_business_classification(self, embedding_matrix): #分类任务
        embedding_list = embedding_matrix.tolist()
