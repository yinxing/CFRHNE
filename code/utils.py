...
def read_graph(g):
    ...
    graph_filename = "../" + config.prefix + "/" + dataset + "/links.dat"
    ...
    adj_g = {}
    c_adj_g = {}
    neg_g = np.zeros((config.node_size, config.node_size))
    cnt = 0
    ...
    with open(graph_filename) as infile:
        for line in infile.readlines():
            ...
            neg_g[source_node][target_node] = 1
            ...
            if(source_node == 0 and target_node == 3906):
                print("ok")
            #dblp            ------------2-----------
            # if(relation == 0):
            #     g_type[0][1] += 1
...
def read_new_embeddings(filename):
    return np.loadtxt(filename)

def read_emb(filename):


    emb = np.zeros((config.node_size, 50))
    cnt = 0

    with open(filename) as infile:
        lines = infile.readlines()[1:]
        for line in lines:
            fetures = line.strip().split()

            emb[int(fetures[0]), :] = str_list_to_float(fetures[1:])
